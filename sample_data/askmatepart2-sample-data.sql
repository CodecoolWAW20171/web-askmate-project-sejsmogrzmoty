...
-    wait patiently for all the girls to reply to the story', NULL);
...
Such a stupendous name as well m''lady. Well tips fedora thanks, and thanks again for helping me join the logang. #logangforlife﻿', NULL);
...
Is there an option / plugin that I can use to be able to rename all times an ID is used in my JavaScript and CSS when I rename it in the HTML file with a refactor?', NULL);
...
In a way that''s not terribly inefficient since I need to do this many times in a loop. I''m assuming that converting them to a Python list and using itertools.product and back to a numpy array is not the most efficient form.', NULL);
...
Is there a way to remove all these signs from the input strings and keeping only the letters & punctuation in the different languages?', NULL);
...
{''mkt_o'': 1, ''mkt_c'': 2}
Which seems like it might be promising but again, fails to meet requirements.', NULL);
...
So do is desugared differently to how the fixity definition naturally works, which is surprising.', NULL);
...
If you want to have it change everywhere, you might just want to do a global find & replace, instead of using the "refactor" option.', NULL);
...
Fuhrman excused his use of the word saying he used nigger in the context of his "bad cop" persona. The popular press reporting and discussing Fuhrman''s testimony substituted the N-word for nigger.', NULL);
INSERT INTO answer VALUES (5, '2016-12-05 14:19:34', 24, 32, 'You are one enormous weirdo. Stop talking shit about the easter bunny. He does not appreciate it.', NULL);
...
array([[1, 4],
       ...
       [2, 5],
       [3, 5]])
See Using numpy to build an array of all combinations of two arrays for a general solution for computing the Cartesian product of N arrays.', NULL);
...
This appears to have some constant-time overhead that makes it run slower than Panzer''s for small inputs. But for larger inputs, in all the tests I ran, it performs just as well as his fastest implementation (cartesian_product_transpose_pp).', NULL);
INSERT INTO answer VALUES (8, '2013-10-18 22:00:34', 22, 36, 'You can just do normal list comprehension in python
x = numpy.array([1,2,3])
y = numpy.array([4,5])
[[x0, y0] for x0 in x for y0 in y]
which should give you
[[1, 4], [1, 5], [2, 4], [2, 5], [3, 4], [3, 5]]', NULL);
...
If you need more information, check out the Java documentation for regexes.', NULL);
...
More information: http://icu-project.org/apiref/icu4j/com/ibm/icu/lang/UProperty.html#EMOJI', NULL);
INSERT INTO answer VALUES (12, '2018-03-27 07:03:14', 29, 38, '{k : {key_1 : v1, key_2 : v2} for k,v1,v2 in zip(idx, l_1, l_2)}', NULL);
...
<interactive>:6:1: error:
    Precedence parsing error
        cannot mix ‘>>=’ [infixl 1] and ‘⬿≫’ [infixr 1] in the same infix expression', NULL);
...
≡ a >> (b >> c)                           -- [Definition]
∎
do-notation de-sugars differently because it has a different goal. Essentially, since do-notation is essentially writing out a lambda, right-association is needed. This is because m >>= (\v -> (...)) is written as do {v <- m; (...)}. As earlier, the de-sugaring of >> here seems to follow >>= for the sake of consistency.', NULL);
...
So, again, the treatment of source files is something you need to find in your compilers documentation.', NULL);
INSERT INTO answer VALUES (17, '2011-04-01 05:35:37', 576, 40, 'Originally from Overv @ reddit.', 'https://i.imgur.com/QlGpd.gif');
...
INSERT INTO comment VALUES (3, 35, NULL, 'The refactor will work if your HTML has a reference to the CSS file, otherwise the refactor won''t be able co-relate', '2018-03-22 17:14:06');
INSERT INTO comment VALUES (4, 30, NULL, 'You are one stupid i-word as in idiot.', '2017-06-06 14:14:42');
INSERT INTO comment VALUES (5, 31, NULL, 'You should kill yourself. Wait, no it is not punishing enough', '2017-06-04 18:33:00');
INSERT INTO comment VALUES (6, 32, NULL, 'WTF did I just read!', '2016-12-04 09:23:04');
INSERT INTO comment VALUES (7, 36, NULL, 'I noticed that the most expensive step in itertools approach is the final conversion from list to array. Without this last step it''s twice as fast as Ken''s example', '2012-06-21 19:09:29');
INSERT INTO comment VALUES (8, NULL, 6, 'An advantage of this approach is that it produces consistent output for arrays of the same size. The meshgrid + dstack approach, while faster in some cases, can lead to bugs if you expect the cartesian product to be constructed in the same order for arrays of the same size.', '2015-07-27 18:35:12');
INSERT INTO comment VALUES (9, NULL, 6, 'I haven''t noticed any case where this approach produces different results from those produced by meshgrid + dstack. Could you post an example?', '2017-07-16 22:26:26');
INSERT INTO comment VALUES (10, 37, NULL, 'what you want to keep?', '2018-03-27 10:07:06');
...
INSERT INTO comment VALUES (14, 37, NULL, 'Please clarify what exactly you want to keep and remove. On the surface the question appears to be clear but because of the complexity of Unicode it is not and because of that it''s impossible to provide a good answer.', '2018-03-27 18:16:47');
INSERT INTO comment VALUES (15, NULL, 9, 'The obvious gap between ASCII alphanumeric characters and emoji is accentated and non-latin letters. Without the OP''s input on these we don''t know whether this is a good answer (not my DV though)', '2018-03-27 15:55:46');
INSERT INTO comment VALUES (16, NULL, 9, 'Yeah I''m curious as to why this would possibly get downvoted. The second I saw this question, a regular expression was the absolute first thing that came to mind (P.S. since he''s looking for standard characters and punctuation, I''d use something like [^\w\^\-\[\]\.!@#$%&*\(\)/+''":;~?,] but that''s just me being robust and trying collect all typical characters that aren''t symbols). Upvoted because this is definitely a potential solution. If he wants to add some other language characters, he can add them to the expression as necessary.', '2018-03-27 16:05:20');
INSERT INTO comment VALUES (17, NULL, 9, 'great punctuation regex example, looks extensive enough to me for some cases. Also maybe people aren''t reading the whole answer then - as stated at the bottom of the answer, p{L} handles non-English alphabetical characters. I hope it''s understood that I can''t list extensively through every non-English alphabet in my answer as that would be impractically verbose.', '2018-03-27 16:09:15');
INSERT INTO comment VALUES (18, NULL, 10, 'Very useful list. Interesting that something called EmojiParser with a method called removeAllEmojis fails to handle these... :-)', '2018-03-27 10:20:05');
INSERT INTO comment VALUES (19, NULL, 10, 'No, since input.codePointAt only looks at up to 2 characters at most which is a constant upper bound. Also (the newly added) i += Character.charCount(cp) skips over all characters that input.codePointAt inspected (minus 1 in some corner cases).', '2018-03-27 11:36:05');
INSERT INTO comment VALUES (20, NULL, 10, 'String.chars() streams over characters not codepoints. There''s a separate method String.codePoints() for that.', '2018-03-27 11:37:21');
...
INSERT INTO comment VALUES (22, NULL, 10, 'This is basically the same approach as used by EmojiParser and it will soon fail for the same reason. New emojis are relatively frequently added to the Unicode character database and if you are now implementing a solution using the currently defined 1644 emojis for a negative rule set, the implementation will fail as soon as new emojis become available.', '2018-03-27 12:04:50');
INSERT INTO comment VALUES (23, NULL, 12, 'Please take a bow. The audience is cheering. Perfect, thanks.', '2018-03-27 07:07:08');
INSERT INTO comment VALUES (24, NULL, 13, 'The first solution brings the advantage of working n keys. Nice!', '2018-03-27 09:54:58');
INSERT INTO comment VALUES (25, 39, NULL, 'Why wouldn''t it be left-associative? You wouldn''t want monad >>= action1 >>= action2 being parsed as monad >>= (action1 >>= action2), would you?', '2018-03-24 21:44:46');
INSERT INTO comment VALUES (26, 39, NULL, 'Also, the fixity of >> doesn''t theoretically matter, since it is associative by the monad laws.', '2018-03-24 21:45:46');
INSERT INTO comment VALUES (27, 39, NULL, 'worth noting that the usual set of actions you''d want to bind is m a, a->m b, b->m c so the right-associated bind wouldn''t typecheck', '2018-03-24 21:50:05');
INSERT INTO comment VALUES (28, 39, NULL, 'For >> it would often be better to associate the other way, but maybe this was chosen for consistency with >>=.', '2018-03-24 21:58:39');
INSERT INTO comment VALUES (29, 39, NULL, 'It would feel weird if >>= and >> did not associate the same way.', '2018-03-24 22:03:47');
INSERT INTO comment VALUES (30, NULL, 14, 'This is the key observation - you couldn''t chain both operators if it wasn''t infixl.', '2018-03-25 09:50:57'); 
